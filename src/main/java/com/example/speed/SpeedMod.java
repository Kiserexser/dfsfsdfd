...
public class SpeedMod implements ModInitializer {
    ...
    static class ClickGUI extends Screen {
        ...
        private void initModules() {
            ...
            for (List<ModuleEntry> list : modules.values()) {
                for (ModuleEntry e : list) toggles.put(e.name, false);
            }
        }
        ...
        public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
            // Полупрозрачное окно (чуть просвечивает)
            drawRoundedWindow(ctx, winX, winY, WIN_W, WIN_H, 14, 0xAA1E1E1E);
            ...
            drawCenteredString(g, "Linxes ClickGUI", winX + WIN_W / 2, winY + 28);

            // Категории с равными отступами
            ...
            if (modList != null) {
                ...
                for (int i = 0; i < visible && scrollOffset + i < modList.size(); i++) {
                    ...
                    boolean state = toggles.getOrDefault(entry.name, false);
                    String stateText = state ? "ON" : "OFF";
                    g.setColor(state ? new Color(0x55FF55) : new Color(0xFF5555));
                    g.setFont(moduleNameFont);
                    int stateX = listX + listW - g.getFontMetrics().stringWidth(stateText) - 10;
                    g.drawString(stateText, stateX, y + 28);
                }
            }
            g.setClip(oldClip);
                ...
                if (modList != null) {
                    for (int i = 0; i < modList.size(); i++) {
                        ...
                        if (mx >= listX && mx <= listX + listW && my >= y && my <= y + itemH) {
                            ModuleEntry entry = modList.get(i);
                            toggles.put(entry.name, !toggles.getOrDefault(entry.name, false));
                            return true;
                        }
                    }
                }
